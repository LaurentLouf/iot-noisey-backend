...
module.exports = db => {
    ...
    db.collection('devices').find({shortID: req.body.id}).toArray(function (error, results) 
      {
        ...
        if ( results.length > 0 )
        {
          ...
          db.collection("data").insert({'deviceFullID' : results[0].fullID, 'data' : req.body.noise, 'date' : new Date()}, function(err, res) { if (err) throw err; } ) ; 
        } 
      }) ;
  });
  ...
  {
    return db.collection('devices').find({fullID:req.params.uid}).toArray(function(err, results) {
      res.json(results);
      return results;
    });
  });
    ...
    db.collection('devices').find({fullID: req.body.id}).toArray(function (error, results) 
      {
        ...
        else
        {
          ...
          message.shortID = req.body.id.slice(0, 5).replace(':', '') ; 
        }

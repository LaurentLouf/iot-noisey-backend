...
module.exports = db => {
  ...
  router.post('/data', (req, res, next) =>  
  {
    ...
    db.collection('devices').find({fullID: parseInt(req.body.id) }).toArray(function (error, results) 
    {
      ...
      {
        db.collection('devices').insert({'fullID' : parseInt(req.body.id), config: {luminosity: 5, offset: 20, sensitivity: 10, updateRate: 2}, 'lastConnection' : new Date() }) ;
        results = [ {fullID : parseInt(req.body.id), config: {luminosity: 5, offset: 20, sensitivity: 10, updateRate: 2} } ] ; 
      }

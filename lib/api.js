...
module.exports = db => {
  ...
  router.post('/data', (req, res, next) =>  
  {
    ...
    db.collection('devices').find({fullID: parseInt(req.body.id) }).toArray(function (error, results) 
    {
      ...
      // A device has not been found, create the device
      if ( results.length == 0 && typeof req.body.id != "undefined" )
      {
        db.collection('devices').insert({'fullID' : parseInt(req.body.id), config: {luminosity: 5, minNoiseLevel: 20, maxNoiseLevel: 60, updateRate: 2}, 'lastConnection' : new Date() }) ;
        results = [ {fullID : parseInt(req.body.id), config: {luminosity: 5, minNoiseLevel: 20, maxNoiseLevel: 60, updateRate: 2} } ] ; 
      }
      // If the device existed or has been created
      if ( results.length > 0 )
      {
